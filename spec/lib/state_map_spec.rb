...
describe StateMap do
  let(:test_state_map)  { StateMap.new }
  let(:test_connection) { test_state_map.connection }
  describe '.initialize' do
    context 'on first connection' do
      ...
      it 'seeds the database with required seed data' do
        expect(test_connection.keys).to contain_exactly(:current_id, :data_store_id_ranges, :starting_id_of_current_data_store)
      end
    end
  ...
  describe '#increment_current_id' do
    it 'increments and returns the current_id field' do
      before_state = test_connection.get(:current_id)
      returned_id = test_state_map.increment_current_id
      after_state = test_connection.get(:current_id)
  ...
  describe '#data_store_for_new_record' do
    ...
    context 'when it is time to rotate files' do
      ...
      it 'closes the current data store and sets the starting_id_of_current_data_store to the id' do
        test_state_map.data_store_for_new_record(5)
        expect(test_connection.get(:data_store_id_ranges)).to eq([0...5])
        expect(test_connection.get(:starting_id_of_current_data_store)).to eq(5)
      end

      it 'returns a path to a new data store' do
        expect(test_state_map.data_store_for_new_record(5)).to eq('./data/test/data_store_1.pstore')
      end
    end
  end
  ...
  describe '#find_data_store_by_id' do
    context 'when no data_stores have yet been closed yet' do # ie, default with fresh db
      it 'returns a path to _0.pstore' do
        expect(test_state_map.find_data_store_by_id(999999)).to eq("./data/test/data_store_0.pstore")
      end
    end
    ...
    context 'when data_store_id_ranges have been set' do
      ...
      context 'for ids in the non-active pstore' do
        ...
        it 'returns a path to the appropriate pstore' do
          expect(test_state_map.find_data_store_by_id(test_id)).to eq("./data/test/data_store_1.pstore")
        end
      end
      ...
      context 'for ids in the active pstore' do
        let(:test_id) { 301 }
        it 'returns a path to the appropriate pstore' do
          expect(test_state_map.find_data_store_by_id(test_id)).to eq("./data/test/data_store_3.pstore")
        end
      end
    end
  end
  ...
  describe '#map_data_stores_by_ids' do
    ...
    context 'when no data_stores have yet been closed yet' do # ie, default with fresh db
      it 'returns a hash with path to _0.pstore as key, all ids as values' do
        expect(test_state_map.map_data_stores_by_ids(test_ids)).to eq({"./data/test/data_store_0.pstore" => test_ids})
      end
    end
    ...
    context 'when data_store_id_ranges have been set' do
      ...
      it 'returns a hash with pstore paths as keys and ids as values' do
        expect(test_state_map.map_data_stores_by_ids(test_ids)).to eq({
  ...
  describe '.all_data_store_paths' do
    context 'when no data_stores have yet been closed yet' do # ie, default with fresh db
      it 'returns an array containing only path to _0.pstore' do
        expect(test_state_map.all_data_store_paths).to eq(["./data/test/data_store_0.pstore"])
      end
    end
    ...
    context 'when data_store_id_ranges have been set' do
      ...
      it 'returns an array containing all pstore paths' do
        expect(test_state_map.all_data_store_paths).to eq([
  ...
  describe 'find_uniq_store_from_compound_key' do
    it 'returns a path with the first four digits of the compound key as the file suffix' do
      expect(test_state_map.find_uniq_store_from_compound_key("2020-03-09.xxx.xxx")).to eq("./data/test/uniq_store_2020.pstore")
    end
  end
end
